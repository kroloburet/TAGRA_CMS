...
///////////////////////////////////////////////////////////////////////////
//подключиться к серверу и базе
   $db=new mysqli($db_host,$db_user,$db_pass,$db_name);
   $db->set_charset('utf8');
   if(mysqli_connect_error()){
    die("<div class='notific_r'>Не удалось соединиться с сервером: ".$mysqli_connect_error()."</div>$bad_msg");
   }
...
//languages ALTER TABLE `tagra_languages` CHANGE `tag` `tag` VARCHAR(20) CHARACTER SET utf8 COLLATE utf8_bin NOT NULL;
...
//директория для загрузки медиа
   if(!mkdir($cms_path.'/upload',0755)){
    die("<div class='notific_r'>Не удалось создать директорию <q>/upload</q> для загрузки медиа</div>$bad_msg");
   }
   echo"<div class='notific_g'>Директория <q>/upload</q> для загрузки медиа успешно создана</div>";

//директория для языка ru (по умолчанию)
   if(!mkdir($cms_path.'/upload/ru',0755)){
    die("<div class='notific_r'>Не удалось создать директорию <q>/upload/ru</q> загрузки медиа языка по умолчанию</div>$bad_msg");
   }
   echo"<div class='notific_g'>Директория <q>/upload/ru</q> загрузки медиа языка по умолчанию успешно создана</div>";
...
//файл конфигурации базы данных CMS
   if(!file_put_contents($cms_path.'/application/config/database.php',$conf_db)){
    die("<div class='notific_r'>Не удалось создать файл конфигурации базы данных CMS <q>/application/config/database.php</q></div>$bad_msg");
   }
...
//файл index.php для корня
   if(!copy($cms_path.'/instal/root_index.php',$cms_path.'/index.php')){
    die("<div class='notific_r'>Не удалось создать файл индекса CMS <q>/index.php</q></div>$bad_msg");
   }
   echo"<div class='notific_g'>Файл индекса CMS <q>/index.php</q> успешно создан</div>";

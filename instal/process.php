...
<html>
  ...
  <body>
    <div id="content">
      ...
      if ($db->connect_errno) {
          die("<div class='notific_r'>Не удалось соединиться с сервером: $db->connect_error</div>$bad_msg");
      }
      $db->set_charset('utf8');
...
ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_bin;")
          or die("<div class='notific_r'>Не удалось создать таблицу <q>$t</q>: $db->error</div>$bad_msg");
      echo "<div class='notific_g'>Таблица <q>$t</q> успешно создана</div>";
...
($id,$date,'','ru','RU',1);");
      if (!$q_c) {
          die("<div class='notific_r'>Не удалось создать таблицу <q>$t</q>: $db->error</div>$bad_msg");
      }
      echo "<div class='notific_g'>Таблица <q>$t</q> успешно создана</div>";
      if (!$q_i) {
          die("<div class='notific_r'>Не удалось записать начальные данные в таблицу <q>$t</q>: $db->error</div>$bad_msg");
      }
      echo "<div class='notific_g'>Начальные данные успешно записаны в таблицу <q>$t</q></div>";
...
('$date','','$index_title','$index_title','','','','$index_layout_l','$index_layout_r','',12,'{$domen}img/tagra_share.jpg','ru');");
      if (!$q_c) {
          die("<div class='notific_r'>Не удалось создать таблицу <q>$t</q>: $db->error</div>$bad_msg");
      }
      echo "<div class='notific_g'>Таблица <q>$t</q> успешно создана</div>";
...
ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_bin;")
      ...
      if (!mkdir($cms_path . '/upload', 0755)) {
          die("<div class='notific_r'>Не удалось создать каталог <q>/upload</q> для загрузки медиа</div>$bad_msg");
      }
      echo "<div class='notific_g'>Каталог <q>/upload</q> для загрузки медиа успешно создан</div>";

      // каталог для языка ru (по умолчанию)
      if (!mkdir($cms_path . '/upload/ru', 0755)) {
          die("<div class='notific_r'>Не удалось создать каталог <q>/upload/ru</q> загрузки медиа языка по умолчанию</div>$bad_msg");
      }
      echo "<div class='notific_g'>Каталог <q>/upload/ru</q> загрузки медиа языка по умолчанию успешно создан</div>";

      // файл конфигурации CMS
      if (!file_put_contents($cms_path . '/application/config/config.php', $conf)) {
          die("<div class='notific_r'>Не удалось создать файл конфигурации CMS <q>/application/config/config.php</q></div>$bad_msg");
      }
      echo "<div class='notific_g'>Файл конфигурации CMS <q>/application/config/config.php</q> успешно создан</div>";

      // файл конфигурации базы данных CMS
      if (!file_put_contents($cms_path . '/application/config/database.php', $conf_db)) {
          die("<div class='notific_r'>Не удалось создать файл конфигурации базы данных CMS "
              . "<q>/application/config/database.php</q></div>$bad_msg");
      }
      echo "<div class='notific_g'>Файл конфигурации базы данных CMS <q>/application/config/database.php</q> успешно создан</div>";

      // файл карты сайта
      if (!file_put_contents($cms_path . '/sitemap.xml', $sitemap)) {
          die("<div class='notific_r'>Не удалось создать файл карты сайта <q>/sitemap.xml</q></div>$bad_msg");
      }
      echo "<div class='notific_g'>Файл карты сайта <q>/sitemap.xml</q> успешно создан</div>";

      // файл index.php для корня
      if (!copy($cms_path . '/instal/root_index.php', $cms_path . '/index.php')) {
          die("<div class='notific_r'>Не удалось скопировать содержимое файла "
              . "<q>/instal/root_index.php</q> в файл <q>/index.php</q></div>$bad_msg");
      }
      echo "<div class='notific_g'>Содержимое для файла индекса CMS <q>/index.php</q> успешно скопировано</div>";

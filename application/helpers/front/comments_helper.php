          ...
          <form class="add_comment_form">
            <label class="input">
              <input type="text" name="name"
                     ...
                     ?  'onkeyup="TUI.Lim(this, ' . $this->conf['name_limit'] . ')"'
                     : '' ) . PHP_EOL?>
            ...
            <label class="textarea">
              <textarea name="comment" rows="5"
                        ...
                        ? 'onkeyup="TUI.Lim(this, ' . $this->conf['text_limit'] . ')"'
                        : '' ?>></textarea>
            </label>
          ...
          const Comments = {
            ...
            hide: function() {
              ...
              if (comments.length > show) {// лимит превышен, нужно скрывать
                ...
                if (!this.go_to()) {comments.slice(show).hide(); def_text = show_text;}
            ...
            rating: function(el) {
              ...
              let self = $(el),
                  ...
                  err = function() {
                    ...
                    box.append(err_msg);
                    setTimeout(function() {clear(); all.removeClass('comment_rating_disable');}, 4000);
                  },
                  ok = function(total) {
              ...
              $.ajax({
                ...
                success: function(resp) {
                  switch (resp.status) {
                    ...
                    default : err(); console.error(`#### TAGRA ERROR INFO ####\n\n${resp}`);
                  }
                },
                error: function() {err();}
              });
            ...
            add: function(form) {
              let f = $(form),
                  ...
                  msg = function(m) {
                   actions_box.html(m);
                   setTimeout(function() {actions_box.html(actions);}, delay);
                  };
              // валидация полей
              if (!/\S/.test(name_val)) {
                msg(
                  `<p class="notific_r mini full">
              ...
              $.ajax({
                ...
                error: function() {
                  msg('<p class="notific_r mini full"><?= addslashes($this->lexic['basic']['server_error']) ?></p>');
                }
              });
            }
            <?php } ?>

...
<h2 class="float_l">Комментарии</h2>
<div class="algn_r"><i class="fa fa-comment">&nbsp;<?=$c?></i></div>
</div>
<?php foreach($q as $k=>$v){if($c>$vsbl&&$k===$vsbl){?>
<div id="comments_more">
<?php }?>
<div class="comment_name">
<div class="float_l"><i class="fa fa-user"></i>&nbsp;<b><?=$v['name']?></b></div>
<div class="algn_r"><time class="fa fa-clock-o">&nbsp;<?=$v['date']?></time></div>
</div>
...
<?php }}if($c>$vsbl){?>
 <div id="comments_more_btn" onclick="opn_cls('comments_more')" class="noprint">Показано <?=$vsbl?> последних, показать\скрыть еще <?=($c-$vsbl)?></div>
<?php }?>
</div>
<?php }?>

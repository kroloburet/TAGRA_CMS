   ...
   <div id="l_r">
    ...
    <div id="l_r_info" class="opn_cls notific_b">
     ...
     <ul>
      ...
      <li>Если в значении нужно использовать HTML - используйте его, но следите за кавычками.</li>
     </ul>
...
;(function($){
 ...
 var _is_php=function(text){return (!/\S\s*<\?php/i.test(text)&&/<\?php/i.test(text));};
 ...
 var _liner=function(){
  ...
  for(var i=1;i<=lines;i++){_l_lines.append(i+'<br>');}
 ...
 var _choice_file=function(){
  ...
  if(_l_file.hash&&_get_hash(text)!==_l_file.hash&&confirm('Файл "'+_l_file.name+'" был изменен!\nСохранить изменения?')){
   if(!_save_file()){_l_list.val(_l_file.path);return false;}
  }
 ...
 var _get_file=function(path){
  ...
  _l_msg('notific_b','<i class="fa fa-spin fa-spinner"></i>&nbsp;&nbsp;загрузка файла "'+name+'"');
  $.ajax({
   ...
   success: function(resp){
    if(_is_php(resp)){
     ...
     _l_msg('notific_g','Файл "'+name+'" готов к редактированию',3000);
    }else{
     _l_msg('notific_r','Ошибка! Не удалось загрузить файл "'+name+'"');
     _clear();
    }
   },
   error:function(){
    _l_msg('notific_r','Не удалось загрузить файл "'+name+'".<br>Возникла ошибка соединения. Повторите попытку.');
   }
  });
 };
 ///////////////////////сохранение файла
 var _save_file=function(){
  ...
  var result=false,
      ...
      .on('click.LR',function(){_l_textarea.val(_l_file.text);$(this).parent('p').remove();return false;});
  if(!_is_php(_l_textarea.val())){
   _l_msg('notific_r','Файл "'+_l_file.name+'" не сохранен!<br>Возможные ошибки:<br>В начале файла не найден обязательный тег <mark>&lt;?php</mark>;<br>Перед тегом <mark>&lt;?php</mark> присутствует текст;<br>Найдено более одного тега <mark>&lt;?php</mark>').append(revoke);
   return false;
  }
  _l_msg('notific_b','<i class="fa fa-spin fa-spinner"></i>&nbsp;&nbsp;сохранение файла "'+_l_file.name+'"');
  $.ajax({
   ...
   success:function(resp){
    switch(resp.status){
     case 'error':_l_msg('notific_r','Файл "'+_l_file.name+'" не сохранен!<br>Возможные ошибки:<br>'+resp.msg).append(revoke);break;
     case 'ok':_l_msg('notific_g','Файл "'+_l_file.name+'" успешно сохранен!',3000);result=true;break;
     default :console.log(resp);
    }
   },
   error:function(){
    _l_msg('notific_r','Файл "'+_l_file.name+'" не сохранен!<br>Возникла ошибка соединения. Повторите попытку.');
   }
  });
  return result;
 };
 ...
 $('.this_form_control').on('click.LR',function(){//сохранение формы редактирования языка
  var self=$(this),
      html=self.html();
  if(_l_file.hash&&_get_hash(_l_textarea.val())!==_l_file.hash){
   self.html('<i class="fa fa-spin fa-spinner"></i>&nbsp;&nbsp;сохранение файла "'+_l_file.name+'"');
   if(!_save_file()){
    self.html('<p class="notific_r mini">Ошибка при сохранении файла "'+_l_file.name+'"!</p>');
    setTimeout(function(){self.html(html);},5000);
    return false;
   }
  }

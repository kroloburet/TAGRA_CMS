...
<div class="sheath">
    <form id="l_r_form" method="POST" action="/admin/language/edit">
        ...
        <div class="touch">
            <h2 class="float_l">Файлы локализации</h2> <i class="fas fa-info-circle TUI_blue"
            ...
            <div id="l_r" hidden>
                ...
                <div id="l_r_info" class="TUI_notice-info">
                    ...
                    <p>Файлы локализации написаны на скриптовом языке PHP. Данные в них представлены в многомерном
                        массиве. Если вы не знакомы с таким представлением данных, внимательно изучите эту справку. Если
                        у вас возникли трудности, или после редактирования возникли ошибки, или локализация работает не
                        корректно &mdash; обратитесь к разработчику системы.</p>
                    <h3>Структура</h3>
                    <hr>
                    <ul>
                        <li><b>Файл должен начинаться с тега</b> <mark>&lt;?php</mark>. Перед ним допускаются только
                            пробелы и пустые строки.</li>
                        ...
                        <li>Например, в группе <mark>'basic'=>[...]</mark> ключ (языковая переменная используемая
                            в шаблоне) 'home' указывает на значение 'Главная'.</li>
                        <li>В пределах каждой группы ключи должны быть уникальны. Не добавляйте и не редактируйте
                            ключи.</li>
                    </ul>
                    <h3>Синтаксис</h3>
                    <hr>
                    <ul>
                        <li>Строки после <mark>//</mark> или <mark>#</mark> до переноса строки считаются комментариями
                            и будут игнорированы.</li>
                        <li>Каждая пара <mark>'ключ'=>'значение'</mark> должны начинаться с новой строки,
                            отформатированы отступами и <b>быть разделены между собой запятыми</b>.</li>
                        <li><b>Ключ и значение обязательно записываются в одинарных или двойных кавычках, не в
                                апострофах</b>.</li>
                        <li>Если в строке значения нужна пара или одна кавычка, вам необходимо следить за тем, чтобы
                            кавычка из внешней пары не попалась внутри. Пример:
                            <mark>'ключ'=>'так <b class="TUI_red">'</b>не<b class="TUI_red">'</b> нужно'</mark>
                            ,
                            <mark>'ключ'=>"тоже <b class="TUI_red">"</b>не<b class="TUI_red">"</b> верно"</mark>
                            ,
                            <mark>'ключ'=>'не<b class="TUI_red">'</b>правильно'</mark>
                            ,
                            <mark>'ключ'=>'а <b class="TUI_green">"</b>так<b class="TUI_green">"</b> верно'</mark>
                            ,
                            <mark>'ключ'=>"пра<b class="TUI_green">'</b>вильно"</mark>.</li>
                        <li>По возможности избегайте в значениях символ <mark>$</mark>. &mdash; Это специальный символ в PHP,
                            обозначает объявление переменной, как например <mark>$lang</mark>.</li>
                        <li>Нигде не используйте: <mark>&lt;?php</mark>, <mark>&lt;?</mark>, <mark>?&gt;</mark>.
                            &mdash; Эти сочетания тоже специальные в PHP.</li>
                        <li>Если в значении все же нужно использовать специальные символы или кавычки, которые будут
                            конфликтовать с внешними, используйте перед каждым таким символом обратный слэш
                            <mark>\</mark>. &mdash; Это называется экранированием. Символ <mark>\</mark>
                            отменяет специальное значение одного символа после него, а сам он в итоге не будет выведен.
                            Например: <mark>'ключ'=>'значение на \$100, it\'s good'</mark>. Результат:
                            <mark>значение на $100, it's good</mark>. Ни в коем случае <b>не экранируйте ни одну из
                                внешних кавычек</b>
                            <mark>'ключ'=><b class="TUI_red">\</b>'нельзя<b class="TUI_red">\</b>'</mark>.
                            Поскольку символ <mark>\</mark> тоже специальный, чтобы использовать его в значении
                            как обычный символ, его нужно так же экранировать. Например:
                            <mark>'ключ'=>'значение\\я'</mark>. Результат: <mark>значение\я</mark>.</li>
                        <li>В некоторых значениях можно встретить <mark>%s</mark>. &mdash; Это специальные метки,
                            вместо которых система подставит переменные или иные данные. Например:
                            <mark>'ключ'=>'Сегодня %s, на часах %s'</mark>. Результат:
                            <mark>Сегодня вторник, на часах 12:07</mark>.
                            <b>Редактируя значения с такими метками учитывайте их смысловую значимость в строке и не
                                удаляйте их.</b></li>
                        <li>Если вы сомневаетесь, будет ли символ интерпретирован PHP как специальный, попробуйте найти
                            этот символ в <a
                                    href="https://ru.wikipedia.org/wiki/%D0%9C%D0%BD%D0%B5%D0%BC%D0%BE%D0%BD%D0%B8%D0%BA%D0%B8_%D0%B2_HTML"
                                    target="_blank">таблице мнемоников</a> и записать его в виде кода.</li>
                        <li>Переносы на новую строку в значении не будут ошибкой. Но по возможности, старайтесь
                            редактировать значения в одну строку.</li>
        ...
        <div class="TUI_fieldset this_form_control">
            <button type="button" onclick="save_changes(form)">Сохранить изменения</button>
...
<style>
    ...
    #l_r_lines, #l_r .TUI_textarea textarea {
        margin: 0;
...
<script>
    ...
    ;(function ($) {
        ...
        const _l_msg = function (style, msg, time) {
            let box = $('#l_r_msg'),
                html = $('<p/>', {class: 'TUI_full mini ' + style, html: msg});
         ...
         * в файле локализации.
         */
        window.save_changes = function (form) {
            let f = $(form),

        ...
        <div class="touch">
            ...
            <div id="l_r" hidden>
                ...
                <div id="l_r_info" class="TUI_notice-b">
                    ...
                    <hr>
                    <ul>
                        <li><b>Файл должен начинаться с тега</b>
                            <mark>&lt;?php</mark>
                            . Перед ним допускаются только пробелы и пустые строки.
                        </li>
                        <li>Файл содержит данные в виде
                            <mark>'ключ'=>'значение'</mark>
                            .
                        </li>
                        <li><b>Редактируйте только значения ключей!</b></li>
                        <li>Данные поделены на группы заключенные в скобки
                            <mark>[ ]</mark>
                            и разделены запятыми.
                        </li>
                        <li>Например, в группе
                            <mark>'basic'=>[...]</mark>
                            ключ (языковая переменная используемая в шаблоне) 'home' указывает на значение 'Главная'.
                        </li>
                    ...
                    <ul>
                        <li>Строки после
                            ...
                            или
                            <mark>#</mark>
                            до переноса строки считаются комментариями и будут игнорированы.
                        </li>
                        <li>Каждая пара
                            <mark>'ключ'=>'значение'</mark>
                            должны начинаться с новой строки, отформатировннны отступами и <b>быть разделены между собой
                                запятыми</b>.
                        </li>
                        ...
                        <li>Если в строке значения нужна пара или одна кавычка, вам необходимо следить за тем, чтобы
                            ...
                            ,
                            <mark>'ключ'=>"пра<b class="TUI_green">'</b>вильно"</mark>
                            .
                        </li>
                        <li>По возможности избегайте в значениях символ
                            <mark>$</mark>
                            . &mdash; Это специальный сомвол в PHP, обозначает объявление переменной, как например
                            <mark>$lang</mark>
                            .
                        </li>
                        <li>Нигде не используйте:
                            <mark>&lt;?php</mark>
                            ,
                            <mark>&lt;?</mark>
                            ,
                            <mark>?&gt;</mark>
                            . &mdash; Эти сочетания тоже специальные в PHP.
                        </li>
                        <li>Если в значении все же нужно использовать специальные символы или кавычки, которые будут
                            конфликтовать с внешними, используйте перед каждым таким символом обратный слеш
                            <mark>\</mark>
                            . &mdash; Это называется экранированием. Символ
                            <mark>\</mark>
                            отменяет специальное значение одного символа после него, а сам он в итоге не будет выведен.
                            Например:
                            <mark>'ключ'=>'значение на \$100, it\'s good'</mark>
                            . Результат:
                            <mark>значение на $100, it's good</mark>
                            . Ни в коем случае <b>не экранируйте ни одну из внешних кавычек</b>
                            <mark>'ключ'=><b class="TUI_red">\</b>'нельзя<b class="TUI_red">\</b>'</mark>
                            . Поскольку символ
                            <mark>\</mark>
                            тоже специальный, чтобы использовать его в значении как обычный символ, его нужно так же
                            экранировать. Например:
                            <mark>'ключ'=>'значение\\я'</mark>
                            . Результат:
                            <mark>значение\я</mark>
                            .
                        </li>
                        <li>В некоторых значениях можно встретить
                            <mark>%s</mark>
                            . &mdash; Это специальные метки, вместо которых система подставит переменные или иные
                            данные. Например:
                            <mark>'ключ'=>'Сегодня %s, на часах %s'</mark>
                            . Результат:
                            ...
                            . <b>Редактируя значения с такими метками учитывайте их смысловую значимость в строке и не
                                удаляйте их.</b></li>
                        <li>Если вы сомневаетесь, будет ли символ интерпритирован PHP как специальный, попробуйте найти
                            этот символ в <a
                                    href="https://ru.wikipedia.org/wiki/%D0%9C%D0%BD%D0%B5%D0%BC%D0%BE%D0%BD%D0%B8%D0%BA%D0%B8_%D0%B2_HTML"
                                    target="_blank">таблице мнемоников</a> и записать его в виде кода.
                        </li>
                        <li>Переносы на новую строку в значении не будут ошибкой. Но по возможности, страрайтесь
                            редактировать значения в одну строку.
                        </li>
                        <li>Если в значении нужно использовать HTML &mdash; используйте его, но следите за кавычками.
                        </li>
                    </ul>
    ...
    ;(function ($) {
        ...
        $('.this_form_control button').on('click.LR', function () {
            ...
            if (_l_file.hash && _get_hash(_l_textarea.val()) !== _l_file.hash) {
                ...
                if (!_save_file()) {
                    self.html(`<p class="TUI_notice-r mini">Ошибка при сохранении файла <q>${_l_file.name}</q>!</p>`);
                    setTimeout(function () {
                        self.html(html);
                    }, 5000);
                    return;

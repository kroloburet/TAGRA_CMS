...
<b class="red">Обязательно для заполнения!</b></pre>
   <label class="input">
    <input type="text" name="title" placeholder="Заголовок страницы. Пример: Книги о море" value="<?=htmlspecialchars($data['title'])?>" onkeyup="lim(this,100)" onchange="translit(this,'#alias');check_title(this,'<?=$data['id']?>','pages','Страница с таким заголовком уже существует!\nИзмените заголовок и продолжайте.')">
   </label>
...
<!--####### Комментарии, видимость... #######-->
  ...
  <div class="button this_form_control">
   <button type="button" onclick="subm(form,s_opts)">Сохранить изменения</button><a href="/admin/page/get_list" class="btn_lnk">Отменить</a>
  </div>
 </form>
</div>

...
<h1><?=$conf_title?></h1>
<div class="sheath">
 <form method="POST" action="<?=base_url('admin/page/edit/'.$id.'/'.$alias)?>" onsubmit="subm(this,s_opts);return false">
...
<b class="red">Обязательно для заполнения!</b></pre>
   <label class="input">
    <input type="text" class="width90" name="title" placeholder="Заголовок страницы. Пример: Книги о море" value="<?=$title?>" onkeyup="lim(this,100)" onchange="translit(this,'#alias');check_title(this,'<?=$id?>','pages','Страница с таким заголовком уже существует!\nИзмените заголовок и продолжайте.')">
   </label>
...
<b class="red">Обязательно для заполнения!</b></pre>
   <label class="textarea">
    <textarea name="description" class="width90 no-emmet" placeholder="Описание страницы (description)" onkeyup="lim(this,250)" rows="3"><?=$description?></textarea>
   </label>
  </div>
...
<!--####### Комментарии, видимость... #######-->
  ...
  <div class="button">
  <input type="submit" value="Сохранить изменения"><a href="<?=base_url('admin/page/get_list')?>" class="btn_lnk">Отменить</a>
  </div>
 </form>
</div>
<script>

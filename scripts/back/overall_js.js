...
//////////////////////////////////////////////////проверка полей на регулярку
function reg_inpt(/*обьект{id поля:/рег.выражение/}*/reg_obj){
  if(typeof reg_obj==='object'){//первый параметр - обьект
  for(var key in reg_obj){//перебор обьекта циклом
   var el=$('[id='+key+']');
   if(reg_obj[key].test(el.val())){
  ...
  alert('В функцию проверки полей "reg_inpt"\nдолжен быть передан объект {id поля:/рег.выражение/}');
 }
}
...
function subm(form/*this - проверяемая форма*/,s_opts/*обьект{имя поля:/рег.выражение/}*/){
 ...
 btn.after('<i class="fa fa-spin fa-spinner"></i>&nbsp;&nbsp;обработка...&nbsp;&nbsp;').remove();
 submit();
}

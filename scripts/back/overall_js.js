  ...
  $('#count_new_comments')
    .load(
      ...
      function(data) {
        ...
        data > 0 ? count.addClass('fas fa-bell red') : count.remove();
      }
    );
});
...
function img_prev(targ, input) {
  ...
  if (i.val()) {
    // поле не пустое
    h.html(`<img src="${i.val()}" style="max-width:350px">`);
    TUI.Hint(t);
  } else {
    // поле пустое
    h.empty();
      ...
      if (el.length > 1) {
        ...
        for (let i = 0; el.length > i; ++ i) {
          if (req[key].test(el[i].value)) {
            // валидация пройдена
            el[i].className = el[i].className.replace(/(\snovalid)/g, '');
          } else {
            // валидация не пройдена
            el[i].className += ' novalid';
            el[i].focus();
            msg();
            return false;
          }
        ...
        if (req[key].test(el.val())) {
          ...
          el.is('select') || el.is('input:file')
            ? el.parent('label').removeClass('novalid')
            : el.removeClass('novalid');
        } else {
          // валидация не пройдена
          el.is('select') || el.is('input:file')
            ? el.parent('label').addClass('novalid').focus()
            : el.addClass('novalid').focus();
          msg();
          return false;
        }
      }
      ...
      switch (resp.status) {
        case 'ok':
          msg(
            ...
            'notific_g',
            1000,
            (resp.redirect ? function(){location.href = resp.redirect;} : null)
          );
          break;
        case 'error':
          msg(`Ой! Ошибка..( Данные не сохранены.<br>
    ...
    success: function(resp) {
      switch (resp) {
        case '1':
          ...
          break;
        case '0':
          process.replaceWith(self.removeClass().addClass('fas fa-eye-slash red'));
          break;
        case 'error':
...
function files(field, lang, user_conf) {
  ...
  function files_notifer() {
    $('.moxman-window-head').after(`<div style="padding:4px;background-color:#ffc0a2">
      Внимание! Не используйте кириллицу и пробелы в именах файлов и папок!</div>`);
  }
}

...
(function($){
 ...
 var _builder=function(){//построитель окна
  ...
  prev=$('<div/>',{class:'FVG_pop_prev fa-angle-left'}).on('click.FVG',function(){
   _prev();
  }),//кнопка "назад"
  next=$('<div/>',{class:'FVG_pop_next fa-angle-right'}).on('click.FVG',function(){
   _next();
  }),//кнопка "вперед"
  ...
  back.add(close).on('click.FVG',function(e){
   if(e.target===this){
    back.remove();
   }
  });//деактивация окна по клику
 };
 var _insert=function(){//вставка в окно текущий элемент галереи
  ...
  switch(_conf.type){//тип галереи
   case 'foto_folder'://галерея из папки с изображениями
    var img=$('<img/>',{class:'FVG_pop_img',src:opt.f_url,alt:opt.f_url}).on('click.FVG',function(){
     _next();
    });
    _content_box.html(img);
    break;
   case 'foto_desc'://галерея изображений с описаниями
    var img=$('<img/>',{class:'FVG_pop_img',src:opt.f_url,alt:opt.f_title}).on('click.FVG',function(){
     _next();
    }),
...
//////////////////////////////////////////////////////
 ...
 var methods={//публичные методы
  init:function(user_conf){//инициализация плагина
   ...
   });//отложеная загрузка изображений
   ...
   return this.each(function(){//добавить события на все элементы
    $(this).on('click.FVG',function(e){//клик запускает окно
     if(_noactive()&&_conf.type!=='video_yt'){
      return false;
     }//элементы расположены в одну колонку
     e.preventDefault();
